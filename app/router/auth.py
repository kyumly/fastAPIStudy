...
async def register(sns_type: SnsType, reg_info: models.UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        is_exist = await is_email_exist(reg_info.email ,session)
        print(reg_info.email, reg_info.pw)
...
async def login(sns_type: SnsType, user_info: models.UserRegister, session: Session = Depends(db.session)):
    if sns_type == SnsType.email:
        is_exist = await is_email_exist(user_info.email,session)
        if not user_info.email or not user_info.pw:
            return JSONResponse(status_code=400, content=dict(msg="Email and PW must be provided'"))
        if not is_exist:
            return JSONResponse(status_code=400, content=dict(msg="NO_MATCH_USER"))
        user = Users.get(email=user_info.email)
        print("패스워드 : ", user_info.pw)
        print("이메일 : ", user.pw)
...
async def is_email_exist(email: str, session: Session):

    # get_email = Users.get(email=email)
    get_email = session.query(Users.email).filter(Users.email == email).count()
    print(get_email)
    if get_email != 0:
       return True
    return False
